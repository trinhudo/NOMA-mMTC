function out = OP_BTEH_MTCD_I(M,PL_I2I,r_t,PA_QoMS,CPA_QoMS,...
    rho_t,eta_t,alpha_t,g_0,lambda_t,R_M_QoMS,OPT)
    ...
    for t = 1:(M-1)
        ...
        SP_MTCDI_TQoM(t+1) = SP_MTCDI_TQoM(t+1)...
            + (brho_t(t+1))*iota_t(t)*CF_X(a_1) + (rho_t(t+1))*iota_t(t)*CF_X(a_2);

        if (a_1 < PA_QoMS(2)/CPA_QoMS(1))
            SP_MTCDI_TQoM(t+1) = SP_MTCDI_TQoM(t+1) + (brho_t(t+1))*biota_t(t)*CF_X(a_3);
        end

        if (a_2 < PA_QoMS(2)/CPA_QoMS(1))
            SP_MTCDI_TQoM(t+1) = SP_MTCDI_TQoM(t+1) + (rho_t(t+1))*biota_t(t)*CF_X(a_4);
        end
    end
    out = 1 - SP_MTCDI_TQoM;
end